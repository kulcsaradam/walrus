...
def run_wasi_tests(engine):
    ...
    if os.system(wasi_clang_path + " --version >/dev/null") == 0 and False:
        ...
        if sys.platform == 'linux':
            ...
            print("Could not identify system: ", sys.platform + ". Exiting now.")
            return
    ...
    if system != "" and not os.path.exists(folder_name):
        ...
        url = ("https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-32/"+folder_name+".tar.gz")
        try:
    ...
    for source in cpp_sources:
        os.system(wasi_clang_path + "  " +join(TEST_DIR,'sources', source) + " " + flags +"  " + " -o " + join(TEST_DIR, "wasm", source + ".wasm"))

    print('Running wasi tests:')
    xpass = glob(join(TEST_DIR, "wasm",'*.wasm')) + glob(join(TEST_DIR, '*.wast'))
