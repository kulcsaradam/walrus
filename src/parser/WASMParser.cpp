...
#undef GENERATE_SIMD_REPLACE_LANE_CODE_CASE
    ...
    void preallocateParameters(std::vector<stackElement *> slots, std::map<size_t, variableRangeInfo> variableRange)
    {
        for (size_t i = 0; i < m_currentFunctionType->param().size(); i++) {
            ...
            if (slots.empty()) {
                stackElement *elem = new stackElement({ reinterpret_cast<stackElement *>(-1), nullptr, static_cast<Walrus::ByteCodeStackOffset>(m_localInfo[i].m_position), 0 });
...
#undef GENERATE_SIMD_REPLACE_LANE_CODE_CASE
        ...
        for (size_t i = 0; i < m_currentFunction->currentByteCodeSize();) {
            ...
            auto current = variableRange.find(i);
            if (current != variableRange.end()) {
                allocateLocal(slots, current->second, offset, free32Slots, free64Slots, free64Slots);
            }
