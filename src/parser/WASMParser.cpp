...
#undef GENERATE_SIMD_REPLACE_LANE_CODE_CASE
    ...
    void optimizeLocals()
    {
        ...
        for (unsigned i = 0; i < paramSize; i++) {
            variableRangeInfo param = variableRangeInfo(m_localInfo[i].m_position, m_localInfo[i].m_valueType);
            param.start = 0;
            variableRange[m_localInfo[i].m_position] = param;
            // variableRange.push_back(variableRangeInfo(m_localInfo[i].m_position, m_localInfo[i].m_valueType));
        }
        ...
        for (auto &constant : m_preprocessData.m_constantData) {
            ...
            // variableRange.push_back(variableRangeInfo(constant.second, constant.first.type()));
        }
...
#undef GENERATE_SIMD_REPLACE_LANE_CODE_CASE
        ...
        std::map<size_t, bool, std::greater<size_t>> setMap;
        ...
        for (auto &variable : variableRange) {
            ...
            std::copy(positions.begin(), positions.end(), variable.second.gets.begin());
            ...
            while (!positions.empty()) {
                ...
                if (closestLabel->first > closestLabel->second) { // Backward jump case.
                    if (closestSet->first > closestLabel->first) {
                        continue;
                    }

                    if (closestSet->first < closestLabel->second) {
                        positions.push_back(closestLabel->second);
                    }

                    if (closestSet->first > closestLabel->second && closestSet->first < closestLabel->first) {
                        positions.push_back(closestLabel->first);
                    }
                    ...
                    if (closestSet->first > closestLabel->first && closestSet->first < closestLabel->second) {
                        positions.push_back(closestLabel->second);
                    }
                }
            }
        ...
        Walrus::ByteCodeStackOffset offset = m_currentFunctionType->paramStackSize();


        std::vector<stackElement *> free32Slots;
        std::vector<stackElement *> free64Slots;
        std::vector<stackElement *> freeV128Slots;

        // Position to store unused elements at.
        // TODO: Possible optimization: life cycle for unused ranges.
        stackElement *unusedVariableElem = new stackElement({ nullptr, nullptr, offset, variableStatus::used32 });
        offset += 4;
        bool noUnusedVar = true;
        ...
        for (auto &range : variableRange) {
            if (range.start == range.end || range.start == SIZE_MAX) {
                ...
                case Walrus::Value::Type::I64: {
                    if (unusedVariableElem->flag == variableStatus::used32) {
                        offset += 4;
                        unusedVariableElem->flag = variableStatus::used64;
                    }
                    break;
                }
                case Walrus::Value::Type::V128: {
                    if (unusedVariableElem->flag == variableStatus::used32) {
                        offset += 12;
                        unusedVariableElem->flag = variableStatus::used128;
                    } else if (unusedVariableElem->flag == variableStatus::used64) {
                        offset += 8;
                        unusedVariableElem->flag = variableStatus::used128;
                    }
                    break;
        ...
        stackElement *stackHead = nullptr;
        stackElement *stackTail = nullptr;
        for (size_t i = 0; i < m_currentFunction->currentByteCodeSize();) {
            ...
            for (auto &range : variableRange) {
                ...
                if (range.end == i) {
                    if (range.assignedPosition == nullptr) {
                        continue;
                    }
                    ...
                    case Walrus::Value::Type::FuncRef:
#endif
                    case Walrus::Value::Type::F32:
                    case Walrus::Value::Type::I32: {
                        stackElement *current = range.assignedPosition;
                        if (current->next != nullptr && current->next->flag == variableStatus::free32 && current->next->pos + 4 % 8 == 0) {
                            stackElement *newElem = new stackElement({ current->prev, current->next->next, current->pos, variableStatus::free64 });
                            if (current->prev != reinterpret_cast<stackElement *>(-1)) {
                                current->prev->next = newElem;
                            }
                            if (current->next->next != nullptr) {
                                current->next->next->prev = newElem;
                            }

                            delete current->next;
                            delete current;
                            free64Slots.push_back(newElem);
                        } else if (current->prev != reinterpret_cast<stackElement *>(-1) && current->prev->flag == variableStatus::free32 && current->prev->pos % 8 == 0) {
                            stackElement *newElem = new stackElement({ current->prev->prev, current->next, current->prev->pos, variableStatus::free64 });
                            if (current->next != nullptr) {
                                current->next->prev = newElem;
                            }
                            if (current->prev->prev != reinterpret_cast<stackElement *>(-1)) {
                                current->prev->prev->next = newElem;
                            }
                            free64Slots.push_back(newElem);
                            ...
                            current->flag = variableStatus::free32;
                            free32Slots.push_back(current);
                        }
                        break;
                    }
#if defined(WALRUS_64)
                    ...
                    case Walrus::Value::Type::I64: {
                        stackElement *current = range.assignedPosition;
                        if (current->next != nullptr && current->next->flag == variableStatus::free64 && current->next->pos + 8 % 16 == 0) {
                            stackElement *newElem = new stackElement({ current->prev, current->next->next, current->pos, variableStatus::free128 });
                            if (current->prev != reinterpret_cast<stackElement *>(-1)) {
                                current->prev->next = newElem;
                            }
                            if (current->next->next != nullptr) {
                            ...
                            delete current->next;
                            delete current;
                            freeV128Slots.push_back(newElem);
                        } else if (current->prev != reinterpret_cast<stackElement *>(-1) && current->prev->flag == variableStatus::free64 && current->prev->pos % 16 == 0) {
                            stackElement *newElem = new stackElement({ current->prev->prev, current->next, current->prev->pos, variableStatus::free128 });
                            if (current->next != nullptr) {
                                current->next->prev = newElem;
                            }
                            if (current->prev->prev != reinterpret_cast<stackElement *>(-1)) {
                            ...
                            current->flag = variableStatus::free64;
                            free64Slots.push_back(current);
                        }
                        break;
                    }
                    case Walrus::Value::Type::V128: {
                        range.assignedPosition->flag = variableStatus::free128;
                        freeV128Slots.push_back(range.assignedPosition);
                        break;
                    }
                    default: {
                        RELEASE_ASSERT_NOT_REACHED();
                ...
                if (range.start == i) {
                    ...
                    case Walrus::Value::Type::F32:
                    case Walrus::Value::Type::I32: {
                        if (!free32Slots.empty() && !range.needsInit) {
                            stackElement *slot = free32Slots.back();
                            free32Slots.pop_back();
                            range.assignedPosition = slot;
                            range.pos = slot->pos;
                            slot->flag = variableStatus::used32;
                        } else if (!free64Slots.empty() && !range.needsInit) {
                            stackElement *slot = free64Slots.back();
                            free64Slots.pop_back();

                            stackElement *newUsed = new stackElement({ slot->prev, nullptr, slot->pos, variableStatus::used32 });
                            stackElement *newUnused = new stackElement({ newUsed, slot->next, static_cast<Walrus::ByteCodeStackOffset>(slot->pos + 4), variableStatus::free32 });
                            newUsed->next = newUnused;

                            if (slot->prev != reinterpret_cast<stackElement *>(-1)) {
                                slot->prev->next = newUsed;
                            }
                            if (slot->next != nullptr) {
                                slot->next->prev = newUnused;
                            }

                            free32Slots.push_back(newUnused);
                            range.assignedPosition = newUsed;
                            range.pos = slot->pos;
                            delete slot;
                        } else if (!freeV128Slots.empty() && !range.needsInit) {
                            stackElement *slot = freeV128Slots.back();
                            freeV128Slots.pop_back();

                            stackElement *newUsed = new stackElement({ slot->prev, nullptr, slot->pos, variableStatus::used32 });
                            stackElement *newUnused = new stackElement({ newUsed, nullptr, static_cast<Walrus::ByteCodeStackOffset>(slot->pos + 4), variableStatus::free32 });
                            stackElement *newUnused64 = new stackElement({ newUnused, slot->next, static_cast<Walrus::ByteCodeStackOffset>(slot->pos + 8), variableStatus::free64 });
                            newUsed->next = newUnused;
                            newUnused->next = newUnused64;

                            if (slot->prev != reinterpret_cast<stackElement *>(-1)) {
                                slot->prev->next = newUsed;
                            }
                            if (slot->next != nullptr) {
                                slot->next->prev = newUnused64;
                            }

                            free32Slots.push_back(newUnused);
                            free64Slots.push_back(newUnused64);
                            range.assignedPosition = newUsed;
                            range.pos = slot->pos;
                            delete slot;
                        } else {
                            stackElement *slot = nullptr;
                            if (stackHead == nullptr) {
                                slot = new stackElement({ reinterpret_cast<stackElement *>(-1), nullptr, offset, variableStatus::used32 });
                                stackHead = slot;
                                stackTail = slot;
                            } else {
                                slot = new stackElement({ stackTail, nullptr, offset, variableStatus::used32 });
                                stackTail->next = slot;
                                stackTail = slot;
                            }
                            offset += 4;
                    ...
                    case Walrus::Value::F64:
                    case Walrus::Value::I64: {
                        if (!free64Slots.empty() && !range.needsInit) {
                            stackElement *slot = free64Slots.back();
                            free64Slots.pop_back();
                            range.assignedPosition = slot;
                            range.pos = slot->pos;
                            slot->flag = variableStatus::used64;
                        } else if (!freeV128Slots.empty() && !range.needsInit) {
                            stackElement *slot = freeV128Slots.back();
                            freeV128Slots.pop_back();

                            stackElement *newUsed = new stackElement({ slot->prev, nullptr, slot->pos, variableStatus::used64 });
                            stackElement *newUnused = new stackElement({ newUsed, slot->next, static_cast<Walrus::ByteCodeStackOffset>(slot->pos + 8), variableStatus::free64 });
                            newUsed->next = newUnused;
                            ...
                            free64Slots.push_back(newUnused);
                            range.assignedPosition = newUsed;
                            range.pos = slot->pos;
                            delete slot;
                        } else {
                            stackElement *slot = nullptr;
                            if (stackHead == nullptr) {
                                slot = new stackElement({ reinterpret_cast<stackElement *>(-1), nullptr, offset, variableStatus::used64 });
                                stackHead = slot;
                                stackTail = slot;
                            } else {
                                slot = new stackElement({ stackTail, nullptr, offset, variableStatus::used64 });
                                stackTail->next = slot;
                                stackTail = slot;
                            }
                            offset += 8;
                    ...
                    case Walrus::Value::Type::V128: {
                        if (!freeV128Slots.empty() && !range.needsInit) {
                            stackElement *slot = freeV128Slots.back();
                            freeV128Slots.pop_back();
                            range.assignedPosition = slot;
                            range.pos = slot->pos;
                            slot->flag = variableStatus::used128;
                        } else {
                            stackElement *slot = nullptr;
                            if (stackHead == nullptr) {
                                slot = new stackElement({ reinterpret_cast<stackElement *>(-1), nullptr, offset, variableStatus::used128 });
                                stackHead = slot;
                                stackTail = slot;
                            } else {
                                slot = new stackElement({ stackTail, nullptr, offset, variableStatus::used128 });
                                stackTail->next = slot;
                                stackTail = slot;
                            }
                            offset += 16;
...
#undef GENERATE_MEMORY_STORE_CODE_CASE
            ...
            for (size_t j = 0; j < m_preprocessData.m_constantData.size(); j++) {
                if (offsets[0] == m_preprocessData.m_constantData[j].second) {
                    setByteCodeDestination(byteCode, constantData[j]);
                    offsets[0] = SIZE_MAX;
                }
                if (offsets[1] == m_preprocessData.m_constantData[j].second) {
                    setByteCodeSrc0(byteCode, constantData[j]);
                    offsets[1] = SIZE_MAX;
                }
                if (offsets[2] == m_preprocessData.m_constantData[j].second) {
                    setByteCodeSrc1(byteCode, constantData[j]);
                    offsets[2] = SIZE_MAX;
                }
                if (offsets[3] == m_preprocessData.m_constantData[j].second) {
                    setByteCodeExtra(byteCode, constantData[j]);
                    offsets[3] = SIZE_MAX;
                }
            }
            ...
            for (auto &range : variableRange) {
                if (range.originalPosition == offsets[0]) {
                    setByteCodeDestination(byteCode, range.pos);
                }

                if (range.originalPosition == offsets[1]) {
                    setByteCodeSrc0(byteCode, range.pos);
                }

                if (range.originalPosition == offsets[2]) {
                    setByteCodeSrc1(byteCode, range.pos);
                }

                if (range.originalPosition == offsets[3]) {
                    setByteCodeExtra(byteCode, range.pos);
                }
            }
        ...
        for (auto &end : ends) {
            for (size_t i = 0; i < end->offsetsSize(); i++) {
                ...
                if (!constant) {
                    for (auto &range : variableRange) {
                        if (range.originalPosition == end->resultOffsets()[i]) {
                            local = true;
                            end->resultOffsets()[i] = range.pos;
                            ...
                            case Walrus::Value::Type::F64: {
                                i++;
                                end->resultOffsets()[i] = range.pos + 4;
                                break;
                            }
                            case Walrus::Value::Type::V128: {
                                for (unsigned idx = 1; idx < 4; idx++) {
                                    i++;
                                    end->resultOffsets()[i] = range.pos + (idx * 4);
                                }
                                break;
                            }
                            default: {
...
#else
                            if (range.type == Walrus::Value::Type::V128) {
                                i++;
                                end->resultOffsets()[i] = range.pos + 8;
                            }
#endif
                            break;
                        }
            ...
            for (size_t j = 0; j < size; j++) {
                ...
                for (size_t k = 0; k < m_preprocessData.m_constantData.size(); k++) {
                    if (m_preprocessData.m_constantData[k].second == stackOffsets[j]) {
                        constant = true;
                        stackOffsets[j] = constantData[k];
                    }
                }

                if (!constant) {
                    for (auto &range : variableRange) {
                        if (range.originalPosition == stackOffsets[j]) {
                            local = true;
                            stackOffsets[j] = range.pos;
                            ...
                            case Walrus::Value::Type::F64: {
                                j++;
                                stackOffsets[j] = range.pos + 4;
                                break;
                            }
                            case Walrus::Value::Type::V128: {
                                for (unsigned idx = 1; idx < 4; idx++) {
                                    j++;
                                    stackOffsets[j] = range.pos + (idx * 4);
                                }
                                break;
                            }
                            default: {
        ...
        for (auto &code : callIndirects) {
            ...
            bool calleeConst = false;
            bool calleLocal = false;
            for (size_t j = 0; j < size; j++) {
                bool local = false;
                bool constant = false;
                ...
                for (size_t k = 0; k < m_preprocessData.m_constantData.size(); k++) {
                    ...
                    if (m_preprocessData.m_constantData[k].second == call->calleeOffset()) {
                        calleeConst = true;
                        call->setCalleeOffset(constantData[k]);
                    }
                }
                ...
                if (!calleeConst) {
                    for (auto &range : variableRange) {
                        if (range.originalPosition == call->calleeOffset()) {
                            calleLocal = true;
                            call->setCalleeOffset(range.pos);
                        }
                    }
                }
            ...
            if (!calleLocal && !calleeConst && (int64_t)(call->calleeOffset() - offsetDifference) >= 0) {
            ...
            for (auto &range : variableRange) {
                ...
                for (size_t k = 0; k < m_preprocessData.m_constantData.size(); k++) {
                    if (m_preprocessData.m_constantData[k].second == brTable->condOffset()) {
                        constant = true;
                        brTable->setCondOffset(constantData[k]);
                    }
                }

                if (!constant) {
                    if (range.pos == brTable->condOffset()) {
                        local = true;
                        brTable->setCondOffset(range.pos);
                        break;
                    }
                }
        ...
        for (auto &range : variableRange) {
            m_localInfo.push_back({ range.type, range.pos });
            m_currentFunction->m_local.push_back(range.type);
        }

        // TODO: Függvénybe kiszervezni a jobb átláthatóság miatt.
        size_t constSize = 0;
        for (auto &range : variableRange) {
            if (range.needsInit) {
                switch (range.type) {
        ...
        for (auto &range : variableRange) {
            ...
            debugInfo.stackPosition = range.pos;

            debugInfo.start = range.start;
            debugInfo.end = range.end;
