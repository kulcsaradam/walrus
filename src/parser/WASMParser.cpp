...
#undef GENERATE_MEMORY_STORE_CODE_CASE
        ...
        case Walrus::ByteCode::GlobalSet128Opcode: {
            ...
            globalSet->setSrcOffset(position);
            break;
        }
        case Walrus::ByteCode::JumpIfTrueOpcode:
        case Walrus::ByteCode::JumpIfFalseOpcode: {
            reinterpret_cast<Walrus::JumpIfFalse *>(byteCode)->setSrcOffset(position);
            break;
        }
        case Walrus::ByteCode::MemoryGrowOpcode: {
            Walrus::MemoryGrow *memGrow = reinterpret_cast<Walrus::MemoryGrow *>(byteCode);
...
#undef GENERATE_SIMD_REPLACE_LANE_CODE_CASE
            ...
            case Walrus::ByteCode::EndOpcode: {
                // naive range findig here instead of later
        ...
        // what if the variable only has sets and no gets, a location to put the value needs tests
        for (auto &var : variableRange) {
            ...
            for (auto &getPos : var.gets) {
                ...
                while (pos > 0 && !var.needsInit) {
                    ...
                    // only need label information, no "block" concept, like forward or backward jump
                    for (auto &block : var.blocksWithRelation) {
        ...
        size_t biggestLocalOffset = offset;
        ...
        offsetDifference = (int64_t)(maxOffset - biggestLocalOffset) > 0 ? maxOffset - biggestLocalOffset : 0;
        ...
        for (auto &data : m_preprocessData.m_constantData) {
            ...
            if (data.first.type() == Walrus::Value::Type::I32 || data.first.type() == Walrus::Value::Type::F32) {
                offset += 4;
            } else {
        ...
        for (size_t i = 0; i < m_currentFunction->currentByteCodeSize();) {
            ...
            // remake to be easier regarding end, call, etc ...
            offsets.clear();
            switch (byteCode->opcode()) {
#define GENERATE_BINARY_CODE_CASE(name, ...) \
    case Walrus::ByteCode::name##Opcode:
...
#undef GENERATE_MEMORY_STORE_CODE_CASE
            ...
            for (size_t k = 0; k < offsets.size(); k++) {
                ...
                if (!local && offsets[k] != SIZE_MAX) {
                    ...
                    //}
                    // case 1: {
                    //    setByteCodeSrc0(byteCode, offsets[0] - offsetDifference);
                    //    break;
                    //}
                    // case 2: {
                    //    setByteCodeSrc1(byteCode, offsets[0] - offsetDifference);
                    //    break;
                    //}
                    // case 3: {
                    //    setByteCodeExtra(byteCode, offsets[0] - offsetDifference);
                    //    break;
                    //}
                    // default: {
                    //    break;
